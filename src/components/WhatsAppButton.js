// SA Global WhatsApp Button Component
...
const WhatsAppButton = () => {
  // Format phone number for WhatsApp (remove any spaces, dashes, or special characters except +)
  const formatPhoneNumber = (phone) => {
    // Remove all non-digit characters except +
    let cleaned = phone.replace(/[^\d+]/g, '');
    
    // If it doesn't start with +, add the country code for Pakistan (+92)
  ...
  const phoneNumber = formatPhoneNumber(content.contactInfo.phone);
  const whatsappMessage = encodeURIComponent("Hello! I'm interested in your educational services. Can you help me with more information?");
